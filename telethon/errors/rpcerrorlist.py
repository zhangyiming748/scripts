...
class FilePartSizeChangedError(BadRequestError):
    def __init__(self, **kwargs):
        super(Exception, self).__init__('The file part size (chunk size) cannot change during upload' + self._fmt_request(kwargs['request']))
...
class ReactionEmptyError(BadRequestError):
    def __init__(self, **kwargs):
        super(Exception, self).__init__('No reaction provided' + self._fmt_request(kwargs['request']))
...
class ScheduleBotNotAllowedError(BadRequestError):
    def __init__(self, **kwargs):
        super(Exception, self).__init__('Bots are not allowed to schedule messages' + self._fmt_request(kwargs['request']))


class ScheduleDateTooLateError(BadRequestError):
    def __init__(self, **kwargs):
        super(Exception, self).__init__('The date you tried to schedule is too far in the future (last known limit of 1 year and a few hours)' + self._fmt_request(kwargs['request']))
...
class SlowModeWaitError(FloodError):
    def __init__(self, **kwargs):
        self.seconds = int(kwargs.get('capture', 0))
        super(Exception, self).__init__('A wait of {seconds} seconds is required before sending another message in this chat'.format(seconds=self.seconds) + self._fmt_request(kwargs['request']))
...
class StickerPngNopngError(BadRequestError):
    def __init__(self, **kwargs):
        super(Exception, self).__init__('Stickers must be a png file but the used image was not a png' + self._fmt_request(kwargs['request']))
...
class TimeoutError(TimedOutError):
    def __init__(self, **kwargs):
        super(Exception, self).__init__('A timeout occurred while fetching data from the worker' + self._fmt_request(kwargs['request']))
